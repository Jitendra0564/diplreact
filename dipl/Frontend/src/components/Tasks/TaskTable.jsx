...
import { MdOutlineAddTask, MdMoreVert, MdClose } from 'react-icons/md';
...
const TaskTable = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const formattedData = response.data.map((task, index) => {
                ...
                const eventDate = task.history.length > 0 ? formatDate(task.history[0].eventDate) : '';
        ...
        try {
            ...
            const updatedTasks = tasks.map((task) => {
                if (task._id === selectedTask._id) {
                    const assignedToUser = users.find(user => user._id === updatedTask.assignedTo);
                    return {
                        ...task,
                        title: updatedTask.title,
                        description: updatedTask.description,
                        ...
                        assignDate: updatedTask.assignDate,
                        DueDate: updatedTask.DueDate,
                        assignedTo: assignedToUser ? assignedToUser.name : task.assignedTo,
                    };
                }
                return task;
            });
        ...
        try {
          ...
          const updatedTasks = tasks.map((task) => {
            if (task._id === selectedTask._id) {
              return {
                ...task,
                rescheduledDate: formatDate(newRescheduledDate), // Assuming you have a function to format the date
              };
            }
            return task;
          });
        ...
        { name: 'eventDate', label: 'Scheduled Date' },
        {
            ...
            options: {
                customBodyRender: (value, tableMeta) => (
                    <div>
                        <MdMoreVert
                            onClick={(event) => handleMenuOpen(event, tasks[tableMeta.rowIndex])}
                        ...
                        >
                            <MenuItem onClick={handleViewTaskDetails}>View Task</MenuItem>
                            <MenuItem onClick={handleOpenTaskUpdateDialog}>Update Task</MenuItem>
                            <MenuItem onClick={handleDeleteConfirmationOpen}>Delete Task</MenuItem>
                            {/* Conditionally render the "Schedule Task" MenuItem */}
                            {tasks[tableMeta.rowIndex].eventDate ? null : (
                                <MenuItem onClick={handleScheduleDialogOpen}>Schedule Task</MenuItem>
                            )}
                            <MenuItem onClick={handleRescheduleDialogOpen}>Reschedule Task</MenuItem>
                            <MenuItem onClick={() => handleOpenHistoryDialog(selectedTask)}>View History</MenuItem>
                        </Popover>
                    </div>
                ),
            },
    ...
    return (
        ...
        <div className="">
            ...
            {/* Task Details Dialog */}
            <Dialog open={openTaskDetailsDialog} onClose={handleCloseTaskDetailsDialog}>
